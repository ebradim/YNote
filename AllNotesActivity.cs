...
namespace Ynote
{
    [Activity(Label = "AllNotesActivity")]
    public class AllNotesActivity : Activity
    {
        protected override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);
            SetContentView(Resource.Layout.all_notes_layout);
            GetNewestNotes();
            FindViewById<Android.Support.V4.Widget.SwipeRefreshLayout>(Resource.Id.pullRefresh).Refresh += (s, args) => { 
                GetNewestNotes();
                FindViewById<Android.Support.V4.Widget.SwipeRefreshLayout>(Resource.Id.pullRefresh).Refreshing = false;
                FindViewById<RadioButton>(Resource.Id.chkNeweset).Checked = true;
            };

            FindViewById<RadioButton>(Resource.Id.chkNeweset).CheckedChange += (s, args) =>
            {
            ...
            FindViewById<RadioButton>(Resource.Id.chkOldest).CheckedChange += (s, args) =>
            {
                if (args.IsChecked == true)
                {
                    GetOldestNotes();
            ...
            FindViewById<RadioButton>(Resource.Id.chkImportnantNotes).CheckedChange += (s, args) =>
            {
                if (args.IsChecked == true)
                {
                    GetImpNotes();
            ...
            FindViewById<RadioButton>(Resource.Id.chkPassword).CheckedChange += (s, args) =>
            {
                if (args.IsChecked == true)
                {
                    GetNotesWithPassword();
                }
            };
        ...
        public void GetNewestNotes()
        {
            FindViewById<LinearLayout>(Resource.Id.flowallnotes).RemoveAllViews();
        ...
        public void GetNotesWithPassword()
        {
            FindViewById<LinearLayout>(Resource.Id.flowallnotes).RemoveAllViews();
            foreach (Note note in from Note item in new SQLiteConnection(CONNECTION.DBPath)
                                      .Table<Note>().Reverse().Where(x => x.Password != string.Empty)
                                  let note = new Note(item.num, item.NoteColor, item.NoteText, item.NoteTime, item.Important, item.Tag, item.Password, item.DateModified,item.NoteTitle,item.AudioPath)
                                  select note)
        ...
        public void GetImpNotes()
        {
            FindViewById<LinearLayout>(Resource.Id.flowallnotes).RemoveAllViews();
            foreach (Note note in from Note item in new SQLiteConnection(CONNECTION.DBPath)
                                      .Table<Note>().Reverse().Where(x => x.Important =="Yes")
                                  let note = new Note(item.num, item.NoteColor, item.NoteText, item.NoteTime, item.Important, item.Tag, item.Password, item.DateModified,item.NoteTitle,item.AudioPath)
                                  select note)
